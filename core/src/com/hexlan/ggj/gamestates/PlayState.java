...
public class PlayState extends GameState {
	...
	public PlayState(GSM gsm) {
		...
		player.setCollisionLayer((TiledMapTileLayer) tiledMap.getLayers()
				.get(1));
		...
		for (int i = 0; i < 4; i++) {
			player.sprites.get(Player.IDLE)[i] = new TextureRegion(temp,
					i * 64, 0, 64, 64);
		}

		temp = new Texture("images/run.png");
		for (int i = 0; i < 6; i++) {
			player.sprites.get(Player.WALKING)[i] = new TextureRegion(temp,
					i * 72, 0, 72, 72);
		}

		temp = new Texture("images/jump.png");
		player.sprites.get(Player.JUMPING)[0] = new TextureRegion(temp, 0, 0,
				72, 72);

		temp = new Texture("images/fall.png");
		player.sprites.get(Player.FALLING)[0] = new TextureRegion(temp, 0, 0,
				72, 72);
	...
	public void update(float dt) {
		...
		arrowCounter++;
		if (arrowCounter >= 180) {
			arrows.add(new Arrow(78, 1340, (TiledMapTileLayer) tiledMap
					.getLayers().get(1), true));
			arrows.add(new Arrow(2270, 910, (TiledMapTileLayer) tiledMap
					.getLayers().get(1), false));
			arrows.add(new Arrow(78, 480, (TiledMapTileLayer) tiledMap
					.getLayers().get(1), true));

			arrows.add(new Arrow(6832, 5135, (TiledMapTileLayer) tiledMap
					.getLayers().get(1), false));
			arrows.add(new Arrow(6832, 5900, (TiledMapTileLayer) tiledMap
					.getLayers().get(1), false));
	...
	public void render(SpriteBatch sb) {
		...
		TiledMapTileLayer objects = (TiledMapTileLayer) tiledMap.getLayers()
				.get(0);

		sb.begin();

		for (int i = 0; i < arrows.size(); i++) {
			if (arrows.get(i).goingRight)
				sb.draw(arrows.get(i).getTexture(),
						arrows.get(i).getX()
								- arrows.get(i).getWidth()
								/ 2
								- (camera.position.x - camera.viewportWidth / 2),
						arrows.get(i).getY()
								- arrows.get(i).getHeight()
								/ 2
								- (camera.position.y - camera.viewportHeight / 2));
			else
				sb.draw(arrows.get(i).getTexture(),
						...
						arrows.get(i).getY()
								...
								/ 2
								- (camera.position.y - camera.viewportHeight / 2),
						-arrows.get(i).getTexture().getWidth(), arrows.get(i)
		...
		for (int x = 0; x < objects.getWidth(); x++) {
			for (int y = 0; y < objects.getHeight(); y++) {
				...
				if (cell != null && cell.getTile() != null
						&& cell.getTile().getProperties().containsKey("torch")) {
					sb.draw(torch.getImage(), x * objects.getTileWidth() + 16
							- (camera.position.x - camera.viewportWidth / 2), y
							* objects.getTileHeight() + 8
							- (camera.position.y - camera.viewportHeight / 2));
				}
				if (cell != null && cell.getTile() != null
						&& cell.getTile().getProperties().containsKey("spike")) {
					sb.draw(spikes, x * objects.getTileWidth()
							- (camera.position.x - camera.viewportWidth / 2), y
							* objects.getTileHeight()
							- (camera.position.y - camera.viewportHeight / 2));
				}
				if (cell != null
						&& cell.getTile() != null
					...
					sb.draw(treasure, x * objects.getTileWidth()
							- (camera.position.x - camera.viewportWidth / 2), y
							* objects.getTileHeight()
							- (camera.position.y - camera.viewportHeight / 2));
				}
				if (cell != null
						...
						&& cell.getTile().getProperties()
								.containsKey("blocks1")) {
					sb.draw(block, x * objects.getTileWidth()
							- (camera.position.x - camera.viewportWidth / 2), y
							* objects.getTileHeight()
							- (camera.position.y - camera.viewportHeight / 2));
				}
				...
				if (cell != null && cell.getTile() != null
						&& cell.getTile().getProperties().containsKey("banner")) {
					sb.draw(banner, x * objects.getTileWidth()
							- (camera.position.x - camera.viewportWidth / 2), y
							* objects.getTileHeight()
							- (camera.position.y - camera.viewportHeight / 2)
							- 48);
		...
		if (player.facingRight) {
			sb.draw(player.animation.getImage(), player.getX()
					+ player.animation.getImage().getRegionWidth() / 2
					- (camera.position.x - camera.viewportWidth / 2),
					player.getY()
							- player.animation.getImage().getRegionHeight() / 2
							- (camera.position.y - camera.viewportHeight / 2),
					-player.animation.getImage().getRegionWidth() * 5 / 4,
					player.animation.getImage().getRegionHeight() * 5 / 4);
		} else {
			sb.draw(player.animation.getImage(), player.getX()
					- player.animation.getImage().getRegionWidth() / 2
					- (camera.position.x - camera.viewportWidth / 2),
