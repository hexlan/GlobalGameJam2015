...
public abstract class Character extends Entity
{
	...
	protected Character()
	{
		...
		facingRight = true;
		respawnSound = Gdx.audio.newSound(Gdx.files.internal("sound/respawn.wav"));
	}
	...
	{
		Point topLeft = new Point(((int)(x) - width/2) / (int)hazardLayer.getTileWidth(), ((int)y - height/2) / (int)hazardLayer.getTileHeight());
		Point bottomRight = new Point(((int)(x) + width/2) / (int)hazardLayer.getTileWidth(), ((int)y + height/2) / (int)hazardLayer.getTileHeight());
		...
		for(int col = topLeft.x; col <= bottomRight.x; col++)
		{
			for(int row = topLeft.y; row <= bottomRight.y; row++)
			{
				...
				if(cell != null && cell.getTile() != null && cell.getTile().getProperties().containsKey("spike")) { respawn(); }
				...
				{ 
					...
					for(int x = 0; x <= hazardLayer.getWidth(); x++)
					{
						for(int y = 0; y <= hazardLayer.getHeight(); y++)
						{
							...
							{
								cell.getTile().getProperties().remove("blocks1");
							}
						}
					}
				}
				...
				{ 
					...
					for(int x = 0; x <= hazardLayer.getWidth(); x++)
					{
						for(int y = 0; y <= hazardLayer.getHeight(); y++)
						{
							...
							{
								cell.getTile().getProperties().remove("blocks2");
							}
						}
					}
				}
				...
				{ 
					...
					for(int x = 0; x <= hazardLayer.getWidth(); x++)
					{
						for(int y = 0; y <= hazardLayer.getHeight(); y++)
						{
							...
							{
								cell.getTile().getProperties().remove("blocks3");
							}
						}
					}
				}
	...
	public void respawn()
	{
		if(x > 2390 && x < 4580 && y > 4580) { setPosition(2423, 4648); }
		if(x > 4577) { setPosition(4742, 4648); }
		if(x < 2390 &&  y < 4648 && y > 2152) { setPosition(1979, 4120); }
		if(x > 2390 && y < 4648) { setPosition(2616, 2344); }
		if(y < 2152) { setPosition(1206, 2152); }
